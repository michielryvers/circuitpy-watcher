...
public sealed class WriteCoordinator
{
    ...
    private readonly Channel<Func<CancellationToken, Task<HttpStatusCode>>> _queue =
        Channel.CreateUnbounded<Func<CancellationToken, Task<HttpStatusCode>>>();
    ...
    {
        return await ExecuteOrQueueAsync(
            ct,
            () => _client.PutDirectoryAsync(dirPath, timestamp, ct)
        );
    }
    ...
    )
    {
        return await ExecuteOrQueueAsync(
            ct,
            () => _client.PutFileAsync(filePath, content, timestamp, ct)
        );
    }
    ...
    )
    {
        return await ExecuteOrQueueAsync(
            ct,
            () => _client.MoveAsync(sourcePath, destinationPath, isDirectory, ct)
        );
    }
    ...
    private async Task<HttpStatusCode> ExecuteOrQueueAsync(
        CancellationToken ct,
        Func<Task<HttpStatusCode>> op
    )
    {
        if (IsPaused)
        {
    ...
    private void EnterPaused()
    {
        lock (_stateLock)
        {
            if (_paused)
                return;
            _paused = true;
            ConsoleEx.Warn("PAUSE (USB MSC active) – waiting for writable…");
            EnsureMonitor();
        }
    }
    ...
    private void ExitPaused()
    {
        ...
        ConsoleEx.Success("RESUME (writable)");
    }
