...
public sealed class LocalWatcher : IDisposable
{
    ...
    private void OnChangedOrCreated(object sender, FileSystemEventArgs e)
    {
        ...
        var t = _debouncers.GetOrAdd(
            path,
            _ => new System.Timers.Timer(_cfg.DebounceMilliseconds) { AutoReset = false }
        );
        t.Stop();
    ...
    {
        if (IgnorePath(e.OldFullPath) && IgnorePath(e.FullPath))
            return;
    ...
    private void OnDeleted(object sender, FileSystemEventArgs e)
    {
        ...
        ConsoleEx.Event("DELETE", Rel(e.FullPath), "skipped by policy");
    }
    ...
    private async Task ProcessQueueAsync(CancellationToken ct)
    {
        while (await _queue.Reader.WaitToReadAsync(ct))
        {
            while (_queue.Reader.TryRead(out var path))
            {
                ...
                {
                    ConsoleEx.Error($"{Rel(path)} ({ex.Message})");
                }
            }
        }
    }
    ...
    private async Task HandleChangeAsync(string path, CancellationToken ct)
    {
        ...
        if (Directory.Exists(path))
        {
            ...
            ConsoleEx.Event("SKIP", rel, "directory change");
            return;
        }
        ...
        if (!File.Exists(path))
        {
            ...
            ConsoleEx.Event("SKIP", rel, "file missing");
            return;
        }
        ...
        if (remoteEntry == null)
        {
            ...
            var status = await _wc.PutFileAsync(
                remoteFile,
                await File.ReadAllBytesAsync(path, ct),
                new DateTimeOffset(localMTimeUtc),
                ct
            );
            ConsoleEx.Action("PUSH", rel, $"reason: missing-remote | {(int)status}");
            return;
        }
        ...
        if (localMTimeUtc > remoteTime || localSize != remoteSize)
        {
            ...
            ConsoleEx.Action("PUSH", rel, $"reason: local-newer|size-diff | {(int)status}");
        }
        else if (remoteTime > localMTimeUtc)
        {
            ...
            if (resp.IsSuccess && resp.Body != null)
            {
                ...
                ConsoleEx.Action("PULL", rel, "reason: remote-newer");
            }
            else
            {
                ConsoleEx.Error(
                    $"{rel} (failed to pull: {(int)resp.StatusCode} {resp.StatusCode})"
                );
            }
        }
        else
        {
            ConsoleEx.Event("SKIP", rel, "equal");
        }
    }

    private async Task HandleRenameAsync(string oldPath, string newPath)
    {
        if (IgnorePath(newPath))
            return;
        ...
        {
            ConsoleEx.Action("MOVE", $"{Rel(oldPath)} -> {Rel(newPath)}", "reason: local-rename");
            return;
        }
        ...
        if (!isDir && File.Exists(newPath))
        {
            ...
            ConsoleEx.Action("MOVE", $"{Rel(oldPath)} -> {Rel(newPath)}", "fallback PUT+DELETE");
        }
        else
        {
            ConsoleEx.Error($"MOVE {Rel(oldPath)} -> {Rel(newPath)} (status {(int)status})");
        }
    }
    ...
    private bool IgnorePath(string fullPath)
    {
        try
        {
            if (PathMapper.IsSymlink(fullPath))
                return true;
